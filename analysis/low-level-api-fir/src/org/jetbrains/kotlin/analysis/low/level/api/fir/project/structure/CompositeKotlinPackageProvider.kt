...
private constructor(
    ...
    override fun getSubPackageFqNames(packageFqName: FqName, platform: TargetPlatform, nameFilter: (Name) -> Boolean): Set<Name> {
        return providers.collectForEach(mutableSetOf()) {
            addAll(it.getSubPackageFqNames(packageFqName, platform, nameFilter))
        }
    }

    override fun getKotlinOnlySubPackagesFqNames(packageFqName: FqName, nameFilter: (Name) -> Boolean): Set<Name> {
        return providers.collectForEach(mutableSetOf()) {
            addAll(it.getKotlinOnlySubPackagesFqNames(packageFqName, nameFilter))
        }
    }
    ...
    override fun getPlatformSpecificSubPackagesFqNames(
        ...
        return providers.collectForEach(mutableSetOf()) {
            addAll(it.getPlatformSpecificSubPackagesFqNames(packageFqName, platform, nameFilter))
        }
    }
