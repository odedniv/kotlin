    ...
    val allPackages: Set<FqName> by lazy {
        buildSet {
            for (extension in extensions) {
                val packages = extension.getContainedPackages()
                addAll(packages)
            }
        }
    }
...
private class LLFirResolveExtensionToolDeclarationProvider(
    ...
    private val extensionFileToKtFile: ConcurrentHashMap<KtResolveExtensionFile, FileBasedKotlinDeclarationProvider> =
        ConcurrentHashMap()
    ...
    private fun createDeclarationProviderByFile(file: KtResolveExtensionFile): FileBasedKotlinDeclarationProvider {
        return extensionFileToKtFile.getOrPut(file) {
