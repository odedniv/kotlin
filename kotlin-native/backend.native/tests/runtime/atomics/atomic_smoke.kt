...
fun testAtomicInt() {
    ...
    assertEquals(18, atomic.get())
    assertEquals(18, atomic.compareAndSwap(18, 56))
    assertEquals(56, atomic.compareAndSwap(18, 56))
    assertEquals(56, atomic.compareAndSwap(18, 56))
}
...
fun testAtomicLong() {
    ...
    assertEquals(1424920024888900000, atomic.get())
    atomic.set(2424920024888900000)
    assertEquals(2424920024888900000, atomic.get())
    assertEquals(2424920024888900000, atomic.getAndSet(3424920024888900000))
    assertEquals(3424920024888900000, atomic.get())
    assertTrue(atomic.compareAndSet(3424920024888900000, 4424920024888900000))
    assertFalse(atomic.compareAndSet(9, 1))
    assertEquals(4424920024888900000, atomic.get())
    assertEquals(4424920024888900000, atomic.getAndAdd(100000))
    assertEquals(4424920024889000000, atomic.get())
    assertEquals(4424920024890000000, atomic.addAndGet(1000000))
    assertEquals(4424920024890000000, atomic.getAndIncrement())
    assertEquals(4424920024890000001, atomic.get())
    assertEquals(4424920024890000002, atomic.incrementAndGet())
    assertEquals(4424920024890000002, atomic.get())
    assertEquals(4424920024890000002, atomic.getAndDecrement())
    assertEquals(4424920024890000001, atomic.get())
    assertEquals(4424920024890000000, atomic.decrementAndGet())
    assertEquals(4424920024890000000, atomic.get())
    assertEquals(4424920024890000000, atomic.compareAndSwap(4424920024890000000, 5424920024890000000))
    assertEquals(5424920024890000000, atomic.compareAndSwap(18, 56))
    assertEquals(5424920024890000000, atomic.compareAndSwap(18, 56))
}
...
fun testAtomicRef() {
    ...
    assertEquals(listOf("a", "b", "c"), atomic.get())
    atomic.set(listOf("a", "b", "a"))
    assertEquals(listOf("a", "b", "a"), atomic.get())
    assertEquals(listOf("a", "b", "a"), atomic.getAndSet(listOf("a", "a", "a")))
    assertEquals(listOf("a", "a", "a"), atomic.get())
    var cur = atomic.get()
    assertTrue(atomic.compareAndSet(cur, listOf("b", "b", "b")))
    assertFalse(atomic.compareAndSet(listOf("a", "a", "a"), listOf("b", "b", "b")))
    assertEquals(listOf("b", "b", "b"), atomic.get())
    cur = atomic.get()
    assertEquals(listOf("b", "b", "b"), atomic.compareAndSwap(cur, listOf("c", "c", "c")))
    assertEquals(listOf("c", "c", "c"), atomic.compareAndSwap(cur, listOf("d", "d", "d")))
    assertEquals(listOf("c", "c", "c"), atomic.compareAndSwap(cur, listOf("c", "c", "c")))
}

@Test
fun testNativePtr() {
    val atomic = AtomicNativePtr(NativePtr.NULL)
    assertEquals(NativePtr.NULL, atomic.get())
    atomic.set(NativePtr.NULL.plus(10L))
    assertEquals(10L, atomic.get().toLong())
    assertTrue(atomic.compareAndSet(NativePtr.NULL.plus(10L), NativePtr.NULL.plus(20L)))
    assertEquals(20L, atomic.get().toLong())
