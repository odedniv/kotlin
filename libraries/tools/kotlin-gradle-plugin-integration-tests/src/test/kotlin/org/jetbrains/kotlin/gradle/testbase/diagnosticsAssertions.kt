...
// NB: Needs verbose mode of diagnostics, see [PropertiesProvider.internalVerboseDiagnostics]
fun extractProjectsAndTheirVerboseDiagnostics(text: String): String = buildString {
        ...
        when {
            line.trim() == VERBOSE_DIAGNOSTIC_SEPARATOR -> {
                if (diagnosticStarted) {
                    ...
                    error("Unexpected end of diagnostic $line on line ${index + 1}. Full text:\n\n" + text)
                }
            }
 ...
 */
val DIAGNOSTIC_START_REGEX = """\s*[we]:\s*\[[^\[]*].*""".toRegex()
...
/*
 ...
 Expected result for this case:
     ...
     Multiline
         Text"
  */
fun extractVerboselyRenderedDiagnostic(diagnostic: ToolingDiagnosticFactory, fromText: String): String? {
