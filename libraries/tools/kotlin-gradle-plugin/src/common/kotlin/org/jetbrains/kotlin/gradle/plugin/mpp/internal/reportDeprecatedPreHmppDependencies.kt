...
internal fun checkAndReportPreHmppDependenciesUsage(
    ...
    for (configuration in configurationsToInspect) {
        configuration.incoming.afterResolve { configuration ->
            // We don't want to report deprecation on transitive dependencies. Unfortunately Gradle doesn't distinguish
            // direct and transitive 'api'-dependencies
            val directDependenciesGavs = configuration.dependencies.mapTo(mutableSetOf()) {
                with(it) { "$group:$name:$version" }
            }
            val resolvedDependencies = configuration.resolutionResult.root.dependencies
                .filterIsInstance<ResolvedDependencyResult>()
                .filter {
                    val selectedGav = (it.selected.id as? ModuleComponentIdentifier)?.run { "$group:$module:$version" }
                    selectedGav in directDependenciesGavs
                }
